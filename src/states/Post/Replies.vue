<template>
	<div class="post-replies">
		...
		<div
			v-for="post in replies.items.slice().reverse()"
			class="post"
		>
			<div class="phi-media">
...
export default {
	...
	data() {
		return {
			replies: app.api.collection(`/people/${app.user.id}/posts/feed/${this.$route.params.postId}/replies`),
			replyBody: "",
			moment
		}
	},
	...
	methods: {
		...
		sendReply() {
			...
			this.replyBody = "";
			this.replies.unshift(outgoing);

			app.api.post(`/posts/${this.$route.params.postId}/replies?all`, outgoing)
			// app.api.post(`/posts/${this.$route.params.postId}/replies`, outgoing)
				.then(post => {
					app.api.clear(`/people/${app.user.id}/posts/feed`);
					app.api.clear(`/people/${app.user.id}/posts/feed/${this.$route.params.postId}/replies`);
					this.replies.splice(outgoing);
					this.replies.unshift(post);
				});
		},
	...
	beforeRouteEnter(to, from, next) {
		var replies = app.api.collection(`/people/${app.user.id}/posts/feed/${to.params.postId}/replies`);
		replies.fetch({type: to.query.type})
			.then(() => {
				next(vm => {
					vm.replies = replies;
				});
